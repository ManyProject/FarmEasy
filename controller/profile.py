...
def get_details():
    query = "SELECT user_image, user_name, user_email, user_phone, user_address, user_role FROM user WHERE user_id = %s"
    try:
        connection = connect()
...
def get_profile():
    ...
    return render_template('profile.html', user_details = user_details, items=items, subtotal=subtotal)
...
def set_profile():
    ...
    query = "UPDATE user SET user_image = %s, user_name = %s, user_phone = %s, user_address = %s \
             WHERE user_id = %s"
    try:
        connection = connect()
        cur = connection.cursor()
        params = (image_path, user_name, user_phone, user_address, session['id'],)
        cur.execute(query, params)
        connection.commit()
    except mysql.connector.Error as err:
        print(err)
...
def get_update_page():
    user_details = get_details()
    items, subtotal, items_len = cart_items()
    return render_template('password.html', user_details=user_details, items=items, subtotal=subtotal)
